				...
				//Hier gehts los
	          	   ...
	          	   if(Main.OwnerIds.contains(m.getIdLong())) {
	           		 ...
	           		 ResultSet set = LiteSQL.onQuery("SELECT * FROM codes WHERE code = '" + code +"'");
	           		  try {
	 					if(set.next()) {
	 						  ch.sendMessage("Ok du hast es geschafft. Ein code gibt es doppelt. Ein mal applaus f�r dich! :) Machs einfach nochmal. Es generieren immer neue Codes").queue();
	 					  }else {
