...
import com.newtime.system.handler.ChattingHandler;
import com.newtime.util.Translations;
import com.newtime.util.checks.CheckMute;
...
public class OnOmeChannelJoin extends ListenerAdapter {
	...
	public static List<Member> echoUser = new ArrayList<>();
			...
			if(joined.getIdLong() == channelid) {
				if(!activeGuilds.contains(joined.getGuild())) {
					...
					if(getUserData.next()) {
						...
						VoiceChannel vc = cat.createVoiceChannel("OmeCord - " + member.getEffectiveName()).complete();
						...
						TextChannel tc = cat.createTextChannel("OmeCord - Chat").complete();
						...
						tc.sendMessage(new EmbedBuilder()
								...
								.build()).setActionRow(
										...
										Button.danger("stop", "Stop (comming soon)"),
										Button.secondary("echo", "Echo test")
										) .queue();
		...
		if(ch.getMembers().size() <= 1) {
			if(omechannels.containsKey(ch.getIdLong())) {
				...
				if(Queue.activeUser.containsKey(user)) {
					...
					Member parter = (Member) randomKey;
					
					ResultSet getUser2Data = LiteSQL.onQuery("SELECT * FROM members WHERE userid = " + parter.getIdLong());
					
					try {
						if(getUser2Data.next()) {
							String lan = getUser2Data.getString("selcLan");
							String gender = getUser2Data.getString("gen");
							
							Queue.addUserToQueue(g1, parter, lan, gender);
							
							if(!Queue.matchUser(g1, (Member) randomKey)) {
